...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    # NEW: Include role from DB in the token payload
    token_payload = {
        "sub": form_data.username,
        "role": user.get("role", "CAMPUS_ADMIN")  # default if older user has no role
    }  # NEW

    token = create_access_token(token_payload)  # CHANGED

    return {"access_token": token, "token_type": "bearer", "role": token_payload["role"]}  # NEW
